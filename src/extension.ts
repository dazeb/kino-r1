...
export async function activate(context: vscode.ExtensionContext) {
  ...
  if (!vscode.workspace.workspaceFolders || vscode.workspace.workspaceFolders.length === 0) {
    vscode.window.showErrorMessage('Kino AI requires an open workspace folder to function.');
    return;
  }
  ...
  // SPECS Commands
  const createNewSpecCommand = vscode.commands.registerCommand('kinoAI.createNewSpec', async () => {
    ...
    if (specName) {
      try {
        ...
        await fileWatcherService.writeFile(`${specPath}/requirements.md`, `# ${specName} - Requirements\n\n## Overview\n\nTODO: Add requirements for ${specName}\n`);
        await fileWatcherService.writeFile(`${specPath}/design.md`, `# ${specName} - Design\n\n## Architecture\n\nTODO: Add design for ${specName}\n`);
        await fileWatcherService.writeFile(`${specPath}/tasks.md`, `# ${specName} - Tasks\n\n## Implementation Tasks\n\nTODO: Add tasks for ${specName}\n`);

        vscode.window.showInformationMessage(`Specification '${specName}' created successfully!`);
      } catch (error) {
  ...
  const openSpecRequirementsCommand = vscode.commands.registerCommand('kinoAI.openSpecRequirements', async (node) => {
    if (node && node.resourceUri) {
      const specName = node.label;
      const requirementsPath = vscode.Uri.joinPath(node.resourceUri, 'requirements.md');
      await vscode.window.showTextDocument(requirementsPath);
    }
  });

  const openSpecDesignCommand = vscode.commands.registerCommand('kinoAI.openSpecDesign', async (node) => {
    if (node && node.resourceUri) {
      const specName = node.label;
  ...
  const openSpecTasksCommand = vscode.commands.registerCommand('kinoAI.openSpecTasks', async (node) => {
    if (node && node.resourceUri) {
      const specName = node.label;
  ...
  const deleteSpecCommand = vscode.commands.registerCommand('kinoAI.deleteSpec', async (node) => {
    if (node && node.resourceUri) {
      const specName = node.label;
      ...
      if (result === 'Delete') {
        try {
          await vscode.workspace.fs.delete(node.resourceUri, { recursive: true });
          vscode.window.showInformationMessage(`Specification '${specName}' deleted successfully!`);
        } catch (error) {
  ...
  // HOOKS Commands
  const createHookCommand = vscode.commands.registerCommand('kinoAI.createHook', async () => {
    ...
    if (hookName) {
      try {
        ...
        await fileWatcherService.writeFile(`hooks/${hookName}.kino.hook`, JSON.stringify(hookConfig, null, 2));
  ...
  const executeHookCommand = vscode.commands.registerCommand('kinoAI.executeHook', async (node) => {
    if (node && node.resourceUri) {
      vscode.window.showInformationMessage(`Executing hook: ${node.label} (functionality will be implemented in Phase 2)`);
    }
  });

  const editHookCommand = vscode.commands.registerCommand('kinoAI.editHook', async (node) => {
    if (node && node.resourceUri) {
      await vscode.window.showTextDocument(node.resourceUri);
    }
  });

  const deleteHookCommand = vscode.commands.registerCommand('kinoAI.deleteHook', async (node) => {
    if (node && node.resourceUri) {
      ...
      if (result === 'Delete') {
        try {
          ...
          vscode.window.showInformationMessage(`Hook '${hookName}' deleted successfully!`);
        } catch (error) {
  ...
  // STEERING Commands
  const createSteeringFileCommand = vscode.commands.registerCommand('kinoAI.createSteeringFile', async () => {
    ...
    if (fileName) {
      try {
        ...
        await fileWatcherService.writeFile(`steering/${fileName}.md`, content);
        vscode.window.showInformationMessage(`Steering file '${fileName}' created successfully!`);
      } catch (error) {
        vscode.window.showErrorMessage(`Failed to create steering file: ${error}`);
      }
    }
  });

  const editSteeringFileCommand = vscode.commands.registerCommand('kinoAI.editSteeringFile', async (node) => {
    if (node && node.resourceUri) {
      await vscode.window.showTextDocument(node.resourceUri);
    }
  });

  const deleteSteeringFileCommand = vscode.commands.registerCommand('kinoAI.deleteSteeringFile', async (node) => {
    if (node && node.resourceUri) {
      ...
      if (result === 'Delete') {
        try {
          await vscode.workspace.fs.delete(node.resourceUri);
          vscode.window.showInformationMessage(`Steering file '${fileName}' deleted successfully!`);
        } catch (error) {
          vscode.window.showErrorMessage(`Failed to delete steering file: ${error}`);
        }
      }
    }
  });
  ...
  // MCP Commands
  const openMCPConfigurationCommand = vscode.commands.registerCommand('kinoAI.openMCPConfiguration', async () => {
    try {
      const mcpConfigPath = vscode.Uri.joinPath(workspaceFolder.uri, '.kino', 'settings', 'mcp.json');
  ...
  // Refresh Commands
  const refreshSpecsCommand = vscode.commands.registerCommand('kinoAI.refreshSpecs', () => {
    specsProvider.refresh();
  });

  const refreshHooksCommand = vscode.commands.registerCommand('kinoAI.refreshHooks', () => {
    hooksProvider.refresh();
  });

  const refreshSteeringCommand = vscode.commands.registerCommand('kinoAI.refreshSteering', () => {
    steeringProvider.refresh();
  });

  const refreshMCPCommand = vscode.commands.registerCommand('kinoAI.refreshMCP', () => {
    mcpProvider.refresh();
  });
  ...
  Logger.info('Chat provider registered.');

  // Show welcome message
  vscode.window.showInformationMessage('Kino AI is now active! Check the sidebar for SPECS, AGENT HOOKS, AGENT STEERING, and MCP SERVERS.');
}
